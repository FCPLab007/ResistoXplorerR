...
InteromicsrCCA <- function(taxrank,generank,ncomp,analmethod,cvmethod,colOpt,colPal,variable,showlabel,imgNm1,format="png",dpi=72){
    ...
    data1<-as.matrix(otu_table(m.data));
    rownames(data1) <- nm;
    m.data.norm <- as.matrix(t(sapply(by(data1,rownames(data1),colSums),identity)));
    r.data <- dataSet$norm.phyobj;
    dataSet$taxa_table <- tax_table(dataSet$taxa_table);
    r.data <- merge_phyloseq(r.data,dataSet$taxa_table);
    ...
    data1<-as.matrix(otu_table(r.data));
    rownames(data1)<- nm;
    r.data.norm <- as.matrix(t(sapply(by(data1,rownames(data1),colSums),identity)));

   # have to filter data (20% rule may be)currently 50%
    r.dataFilt <- t(r.data.norm[rowSums(r.data.norm != 0) > ncol(r.data.norm)/5,]);
    m.dataFilt <- t(m.data.norm[rowSums(m.data.norm != 0) > ncol(m.data.norm)/5,]);
    suppressMessages(require("mixOmics"));
    if (analmethod == "shrinkage"){
        res <- rcc(r.dataFilt,m.dataFilt, ncomp = ncomp, method = 'shrinkage');
    } else {
        cv <- tune.rcc(r.dataFilt,m.dataFilt, grid1 = seq(0.001, 1, length = 5), grid2 = seq(0.001, 1, length = 5), 
            validation = cvmethod,folds = 5, plot=FALSE);
        res <- rcc(r.dataFilt, m.dataFilt, ncomp = ncomp, method = analmethod,lambda1 = cv$opt.lambda1,lambda2 = cv$opt.lambda2);       
    }
    ...
    if (colOpt!="omics"){        
        rcc.res.data$variates$X[,variable] <- sample_data(dataSet$norm.phyobj)[m_sam_nm][[variable]]; 
        rcc.res.data$variates$Y[,variable] <- sample_data(dataSet$m.norm.phyobj)[m_sam_nm][[variable]];
    }
    ...
    dataSet$cca.X <- r.dataFilt;
    dataSet$cca.Y <- m.dataFilt;
    variable <<-variable;
    dataSet <<- dataSet;
    return(1);
}
...
PerformintgromsPLS <- function(taxrank,generank,ncomp,analmethod,colOpt,colPal,variable,showlabel,imgNm1,format="png",dpi=72){
    ...
    m.dataFilt <- t(m.data.norm[rowSums(m.data.norm != 0) > ncol(m.data.norm)/5,]);
    suppressMessages(require("mixOmics")); 
    res <- spls(r.dataFilt,m.dataFilt, ncomp = ncomp, mode = analmethod);
    spls.res.data <- res;
    Cairo(file=imgNm1, width=720, height=500, type=format, bg="white",dpi=dpi);    
    suppressMessages(require(RColorBrewer));      
    m_sam_nm <- sample_names(dataSet$m.norm.phyobj);
    ...
    if (colOpt!="omics"){        
        spls.res.data$variates$X[,variable] <- sample_data(dataSet$norm.phyobj)[m_sam_nm][[variable]]; 
        spls.res.data$variates$Y[,variable] <- sample_data(dataSet$m.norm.phyobj)[m_sam_nm][[variable]];
    }
